# Polynomial Regression Analysis of Bitcoin Returns vs Candidate Odds
...
for candidate in ['Donald Trump', 'Kamala Harris']:
    X = merged_df['Close'].values.reshape(-1, 1)
    ...
    plt.plot(merged_df['Close'], y_pred, color='red', linewidth=2, label='Polynomial Fit (Degree 2)')
    plt.title(f'Bitcoin Price vs {candidate} Probability (Polynomial Fit)', fontsize=16)
    plt.xlabel('Bitcoin Price', fontsize=14)
    plt.ylabel(f'{candidate} Probability (%)', fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{images_path}/polynomial_regression_bitcoin_odds_{candidate.replace(' ', '_').lower()}.png", dpi=300, bbox_inches='tight')
    plt.show()
